...
/**
 * CategorySearchModal Component
 * Provides AI-powered search within a specific blog category
 * Opens via Ctrl+K (Windows) / Cmd+K (Mac) or programmatically
 */
        ...
        <ModalHeader className="flex items-center justify-between border-b dark:border-slate-800 border-slate-200">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-xs text-slate-500">in {typeName}</span>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        <ModalBody className="flex flex-col gap-4 py-4">
          ...
          {!query && !isLoading && (
            <div className="text-center py-12 text-slate-500 dark:text-slate-400">
              <p className="text-sm">
                Start typing to search {typeName.toLowerCase()} articles and get AI suggestions
              </p>
            </div>
          )}
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid gap-3">
                    {results.map((result) => (
                      <Link
                        key={result.id}
                        href={`/${typeName.toLowerCase()}/${result.slug}`}
                        onClick={handleClose}
                      >
